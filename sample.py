...
class sampleMilter(Milter.Milter):
    ...
    def log(self, *msg):
        print(f"{strftime('%Y%b%d %H:%M:%S')} [{self.id}]", end=None)
        for i in msg:
            try:
                print(i, end=None)
    ...
    def envfrom(self, f, *text):
        ...
        if self.user:
            self.log("user", self.user, "sent mail from", f, text)
        else:
            self.log("mail from", f, text)
        return Milter.CONTINUE

    def envrcpt(self, to, *text):
        # mail to MAILER-DAEMON is generally spam that bounced
        if to.startswith("<MAILER-DAEMON@"):
            self.log("DISCARD: RCPT TO:", to, text)
            return Milter.DISCARD
        self.log("rcpt to", to, text)
        return Milter.CONTINUE
    ...
    def header(self, name, val):
        ...
        if lname == "x-mailer" and val.lower() in (
            b"direct email",
            b"calypso",
            b"mail bomber",
        ):
    ...
    def abort(self):
        self.log(f"abort after {self.bodysize} body chars")
        return Milter.CONTINUE
...
if __name__ == "__main__":
    ...
    print(
        f"""To use this with sendmail, add the following to sendmail.cf:

O InputMailFilters=pythonfilter
Xpythonfilter,        S=local:{socketname}
