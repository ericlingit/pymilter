...
def inet_ntop(s):
    ...
    for l in (7, 6, 5, 4, 3, 2, 1):
        ...
        for i in range(9 - l):
            if a[i : i + l] == e:
                if i == 0:
                    return ":" + (":{}" * (8 - l)).format(*a[l:])
                if i == 8 - l:
                    return ("{}:" * (8 - l)).format(*a[:-l]) + ":"
                return ("{}:" * i).format(*a[:i]) + (":{}" * (8 - l - i)).format(
                    *a[i + l :]
                )
    return "{}:{}:{}:{}:{}:{}:{}:{}".format(*a)
    ...
    try:
        ...
        if len(a) == 2:
            ...
            if not l:
                r = r.split(":")
                return struct.pack(
                    "!HHHHHHHH", *[0] * (8 - len(r)) + [int(s, 16) for s in r]
                )
            if not r:
                l = l.split(":")
                return struct.pack(
                    "!HHHHHHHH", *[int(s, 16) for s in l] + [0] * (8 - len(l))
                )
            l = l.split(":")
            r = r.split(":")
            return struct.pack(
        ...
        if len(a) == 1:
            return struct.pack("!HHHHHHHH", *[int(s, 16) for s in a[0].split(":")])
    except ValueError:
        pass
    raise ValueError(p)
