...
class Record(object):
    ...
    def __str__(self):
        return "Grey[{0}:{1}:{2}:{3}]".format(
            time.ctime(self.firstseen),
            time.ctime(self.lastseen),
            self.umis,
            self.cnt,
        )
    ...
    def check(self, ip, sender, recipient, timeinc=0):
        ...
        try:
            ...
            try:
                ...
                if now > r.lastseen + self.greylist_retain:
                    # expired
                    log.debug(f"Expired greylist: {key}")
                    r = Record(timeinc)
                elif now < r.firstseen + self.greylist_time + 5:
                    # still greylisted
                    log.debug(f"Early greylist: {key}")
                    # r = Record(timeinc)
                    r.lastseen = now
                    ...
                    r.lastseen = now
                    r.cnt += 1
                    log.debug(f"Active greylist({r.cnt}): {key}")
                else:
                    # passed greylist window
                    log.debug(f"Late greylist: {key}")
                    r = Record(timeinc)
                dbp[key] = r
            except:
                r = Record(timeinc)
