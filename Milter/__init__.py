...
def nocallback(func):
  try:
    ...
    raise ValueError(f'@nocallback applied to non-optional method: {func.__name__}')
  @wraps(func)
  def wrapper(self, *args):
    if func(self,*args) != CONTINUE:
      raise RuntimeError(f'{func.__name__} return code must be CONTINUE with @nocallback')
    return CONTINUE
  return wrapper
...
def noreply(func):
  try:
    ...
    raise ValueError(f'@noreply applied to non-optional method: {func.__name__}')
  @wraps(func)
  def wrapper(self, *args):
    rc = func(self, *args)
    if self._protocol & nr_mask:
      if rc != CONTINUE:
        raise RuntimeError(f'{func.__name__} return code must be CONTINUE with @noreply')
      return NOREPLY
    return rc
...
def symlist(*syms):
  def setsyms(func):
    if len(syms) > 5:
      raise ValueError(f'@symlist limited to 5 macros by MTA: {func.__name__}')
    if func.__name__ not in MACRO_CALLBACKS:
      raise ValueError(f'@symlist applied to non-symlist method: {func.__name__}')
    func._symlist = syms
    return func
  return setsyms
...
class Base(object):
  ...
  def setreply(self, rcode, xcode=None, msg=None, *ml):
    for m in (msg,) + ml:
      if 1 in [len(s) & 1 for s in R.findall(m)]:
        raise ValueError(f"'%' must be doubled: {m}")
  ...
  def addheader(self, field, value, idx=-1):
    if not self._actions & ADDHDRS:
      raise DisabledAction('ADDHDRS')
  ...
  def chgheader(self, field, idx, value):
    if not self._actions & CHGHDRS:
      raise DisabledAction('CHGHDRS')
  ...
  def addrcpt(self, rcpt, params=None):
    if not self._actions & ADDRCPT:
      raise DisabledAction('ADDRCPT')
  ...
  def delrcpt(self, rcpt):
    if not self._actions & DELRCPT:
      raise DisabledAction('DELRCPT')
  ...
  def replacebody(self, body):
    if not self._actions & MODBODY:
      raise DisabledAction('MODBODY')
  ...
  def chgfrom(self, sender, params=None):
    if not self._actions & CHGFROM:
      raise DisabledAction('CHGFROM')
  ...
  def quarantine(self, reason):
    if not self._actions & QUARANTINE:
      raise DisabledAction('QUARANTINE')
...
class Milter(Base):
  ...
  def connect(self, hostname, family, hostaddr):
    ...
    print(f'connect from {hostname} at {hostaddr}')
    return CONTINUE
  ...
  def envfrom(self, f, *text):
    ...
    text -> tuple	additional ESMTP parameters
    '''
    print(f"mail from {f} {text}")
    return CONTINUE

  @noreply
  def envrcpt(self, to, *text):
    'Called for each message recipient.'
    print(f'rcpt to {to} {text}')
    return CONTINUE
...
# @since 0.9.3
# @param text list of param strings of the form "NAME" or "NAME=VALUE"
# @return a dictionary of ESMTP param names and values
def param2dict(text): 
  'Convert ESMTP parm list to keyword dictionary.'
  pairs = [x.split('=', 1) for x in text]
  for e in pairs:
