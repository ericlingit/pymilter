...
def send_dsn(mailfrom, receiver, msg=None, timeout=600, session=None, ourfrom=""):
    ...
    try:
        ...
        return (450, f"DNS Timeout: {domain} MX")  # temp error
    if not mxlist:
    ...
    for prior, host in mxlist:
        try:
            ...
            a = resp.split()
            if not a:
                return (553, f"MX for {domain} has no hostname in banner: {host}")
            if a[0] == receiver:
                return (553, f"Fraudulent MX for {domain}: {host}")
            if not (200 <= code <= 299):
                raise smtplib.SMTPHeloError(code, resp)
            if msg:
                try:
                    smtp.sendmail(f"<{ourfrom}>", mailfrom, msg)
                except smtplib.SMTPSenderRefused:
                    # does not accept DSN, try postmaster (at the risk of mail loops)
                    smtp.sendmail(f"<postmaster@{receiver}>", mailfrom, msg)
            else:  # CBV
                code, resp = smtp.docmd(f"MAIL FROM: <{ourfrom}>")
                if code != 250:
                    raise smtplib.SMTPSenderRefused(code, resp, f"<{ourfrom}>")
        ...
        if hasattr(smtp, "sock"):
            smtp.close()
        if time.time() > toolate:
            return (450, f"No MX response within {(timeout / 60.0):f} minutes")
...
def create_msg(v, rcptlist=None, origmsg=None, template=None):
    ...
    if "from" not in msg:
        msg.add_header("From", f"postmaster@{v.receiver}")
